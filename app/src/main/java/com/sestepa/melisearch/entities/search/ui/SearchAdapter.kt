...
class SearchAdapter(private val result: SearchData, private val onClickListener: (ProductData) -> Unit): RecyclerView.Adapter<SearchViewHolder>() {
	...
	override fun onBindViewHolder(holder: SearchViewHolder, position: Int) = holder.render(result.products[position], onClickListener)

	override fun getItemCount() = result.products.size
}
